...
3. **Checks whether it's already synced — by the source event's ID, not its title.** Each synced booking carries the ID of Chris's calendar event, stamped invisibly in the business event's description. Before creating anything, the sync looks for an existing business event with that same ID. If it finds one, the booking is already synced and no copy is made. Because the match is on the ID (not title + time), a booking Chris *renamed* or *moved* is still recognized as the same one — see step 4.

4. **Creates the event, or updates the existing copy.** If it's a new, address-matching booking, it creates an event on the business calendar named `[Chris Connelly] <original event title>`, with the same start time, end time, and location, and stamps Chris's event ID into the description. If the booking was already synced but Chris has since **renamed it, moved it to a different time, or changed the room**, the existing business event is edited to match, and its sheet row's Event Date is updated if the time changed. (A booking synced before ID-stamping existed, and still carrying no ID, is adopted on an exact title + time match and stamped, rather than duplicated.)
...
6. **Checks for cancellations, once an hour.** (Live as of this update — separate function, `checkForCancellations`, on its own hourly trigger.) His cancellation policy: cancel 72+ hours before the session and it's really cancelled; cancel with less notice than that and he's still billed. So every hour, for each upcoming synced booking:
   ...
   This only works for bookings that carry a "Source Event ID" in their description (step 4). Bookings synced before ID-stamping existed had none; the one-time `backfillSourceEventIds()` function stamps IDs onto that backlog by matching each unstamped business event to Chris's calendar (same start time, same-or-contained title). Whatever it still can't match — genuine cancellations and reschedules from that era — is cleaned up with `previewChrisOrphanCleanup()` / `applyChrisOrphanCleanup()` (deletes unstamped `[Chris Connelly]` events 72+ hours out with no matching event on Chris's calendar, plus their sheet rows).
...
| **Match is by source event ID** | Reliable across renames and reschedules. The only events it can't match this way are pre-ID-stamping ones that `backfillSourceEventIds()` couldn't resolve — those need the manual orphan cleanup. |
| **One-directional** | Nothing flows back to Chris's calendar. On his side → the business copy's title, time, and location are kept in sync, and a 72+ hour cancellation deletes it; the business event is otherwise never edited. |
...
| **Cancellation detection needs the Source Event ID** | New syncs always carry it. The pre-existing backlog is covered once `backfillSourceEventIds()` has been run; anything it couldn't match is handled by the one-time orphan cleanup, not the hourly check. |
